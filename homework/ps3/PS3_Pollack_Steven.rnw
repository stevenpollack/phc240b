...
\begin{enumerate}
  ...
  \item[d)]
  <<prob2d,results='asis'>>=
  load(file="simstudy.Rdata")
  colnames(sim.study.out) <- paste("n=",n,sep="")
  rownames(sim.study.out) <- c("uniform", "pointwise")
  print(xtable(sim.study.out,caption="Empirical coverage probability for different types of confidence bands; $n$ is number of sampled $T_i$'s, $m$ is number of sampled $U_{i}$'s.",digits=3))
  @
  As table 2 indicates, the coverage for the band created by slapping together 95\% confidence intervals is well under 95\% (in fact, it seems to converge around \Sexpr{round(sim.study.out[2,3]*100)}\%). Alternatively, and unsurprisingly, the uniform confidence band developed in parts a through c has coverage hanging slightly under 95\%.
  
  If you change the number of $U_{i}$'s to 5000, and consider $n=2000$, one finds empirical coverage probabilities around 94\% and 73\% for the uniform and pointwise bands (respectively). This gives us reason to believe the uniform band construction works as advertised.
...
\intertext{Thus, if we perform integration by parts with $u = S(t|z) Q(t|z)$ and $dv = \lambda(t|z) dt$, we have}
  ...
  Now, my claim is that for fixed $z$, \[\sqrt{n}(\hat{\psi}(z)- \psi(z)) \xrightarrow{\L}N(0, \psi(z)(1-p_z))\] To see this, first observe that the central limit theorem implies that $\sqrt{n}(\hat{p}_z - p_z) \xrightarrow{\L} N(0,p_z(1-p_z))$, and the weak law of large numbers implies that $\hat{\mu}_z \xrightarrow{P} \mu_z$. Thus, an application of Slutsky's theorem has that 
  \[
  \frac{\sqrt{n}(\hat{p}_z - p_z)}{\hat{\mu}_z} = \sqrt{n}\left(\hat{\psi}(z) - \frac{p_z}{\hat{\mu}_z}\right) \xrightarrow{\L} N(0,\psi(z)(1-p_z))
  \]
  Thus,
  \[
  \underbrace{\sqrt{n}\left(\hat{\psi}(z) - \psi(z)\right)}_{X_n} + \underbrace{\sqrt{n}\left( \psi(z) - \frac{p_z}{\hat{\mu}_z}\right)}_{Y_n} \xrightarrow{\L} N(0,\psi(z)(1-p_z))
  \]
  But, if we set $X_n \xrightarrow{\L} X$ and observe that $Y_n \xrightarrow{P} 0$, then Slutsky's theorem says that $X_n + Y_n \xrightarrow{\L} X$, which proves the asymptotics claimed earlier.
 ...
 nelson.aalen.estimator <- Vectorize(function(t) {
   out <- sapply(X=1:length(baseline.pop$days),FUN=function(i) {
     num <- as.numeric((baseline.pop$days[i] <= t) & baseline.pop$died[i] )
     denom <- sum( (baseline.pop$days >= baseline.pop$days[i]) )
     num/denom
   })
   sum(out)
 })
  ### ^^ this could be done in a slick way.
  ...
  psi.hat.star <- Vectorize(function(z,data=melanoma) {
    ...
    num <- sum(subpop$status != 2)
    denom <- sum(nelson.aalen.estimator(subpop$days))
    return(num/denom)
  })
  prob4f.out <- cbind(cov.levels,psi.hat.star(0:3))
  prob4f.out <- cbind(prob4f.out,cov.levels %*% coxph(mel.surv.obj~ulc+sex)$coefficients)
